...
/**
 * a stepping slider for the time when the height/width(horizontal/vertical) of the widgets are not equal but we want the widgets to be in the centre
 * of the slider based on each widget's size.
 *
 * @param <T>
 */
public class AlignedSteppingSlider<T extends LoadableImage> extends SteppingSlider<T> {
    public AlignedSteppingSlider(float x, float y, float w, float h, float widgetW, float widgetH, Game game, boolean areWidgetsDraggable, boolean isHorizontal, float gap, boolean playSound) {
        super(x, y, w, h, widgetW, widgetH, game, areWidgetsDraggable, isHorizontal, gap, playSound);
    }

    public AlignedSteppingSlider(float x, float y, float w, float h, float widgetW, float widgetH, Game game, boolean areWidgetsDraggable, boolean isHorizontal, float gap, float allowedEmptySpaceMultiplier, float speedThreshold, boolean playSound) {
